...
static void send_and_handle_cmd(int fd, p_gpio_packet_type type,
                               void *data, size_t size) {
    p_gpio_replay_head replay_head;
    uint8_t buff[256];

    p_gpio_cmd_head cmd_head = {
        .type = type,
        .pin = 23u,
        .size = sizeof(p_gpio_pin_mode),
    };

    send(fd, &cmd_head, sizeof(p_gpio_cmd_head), 0);
    send(fd, data, size, 0);
    recv(fd, &replay_head, sizeof(p_gpio_replay_head), MSG_WAITALL);
    recv(fd, buff, replay_head.size, MSG_WAITALL);
}
...
static void blink(int sock_fd) {
    ...
    while (1) {
        state = !state;
        send_and_handle_cmd(sock_fd, P_GPIO_CMD_WRITE, &state, sizeof(state));
    }
}
...
int main(void) {
    ...
    sock_fd = connect_to_gpio_server(SOCK_PATH);
    blink(sock_fd);
    close(sock_fd);
