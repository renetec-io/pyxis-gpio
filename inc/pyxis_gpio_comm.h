...
#ifndef PYXIS_GPIO_COMM_H
#define PYXIS_GPIO_COMM_H

#include <stdint.h>

#define DEFAULT_GPIO_SOCK_PATH "/tmp/pyxis-gpio-socket"
...
typedef enum {
  ...
  // data: gpio_reply_head + int32_t status
  P_GPIO_REPLY_SET_PIN_MODE = 1024u,
  // data: gpio_reply_head + int32_t status
  P_GPIO_REPLY_SET_PIN_PUD,
  // data: gpio_reply_head + int32_t status
  P_GPIO_REPLY_READ,
  // data: gpio_reply_head + int32_t status
  P_GPIO_REPLY_WRITE,
  // data: gpio_reply_head + int32_t status
  P_GPIO_REPLY_SET_PWM,
  // data: gpio_reply_head
  P_GPIO_REPLY_UNDEFINED_CMD,
} p_gpio_packet_type;
...
/**
 * @brief Head for reply between pyxis-gpio service and clients.
 */
typedef struct {
  p_gpio_packet_type type;
  uint16_t size;
