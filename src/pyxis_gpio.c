...
static ssize_t send_reply(int fd, p_gpio_packet_type type, void *buff, size_t size)
{
  ssize_t bytes = -1;
  p_gpio_reply_head head = {
    .type = type,
    .size = size,
  };

  while ((bytes = send(fd, &head, sizeof(p_gpio_reply_head), 0)) !=
         sizeof(p_gpio_reply_head)) {
    if (errno == EAGAIN) {
      errno = 0;
      continue;
    }
...
static void handle_cmd(gpio_data *p_d)
{
      ...
      if (pfds[i].revents & POLLIN) {
        ...
        switch (cmd_head.type) {
          case P_GPIO_CMD_SET_PIN_MODE:
          {
            ...
            send_reply(pfds[i].fd, P_GPIO_REPLY_SET_PIN_MODE, &status, sizeof(int32_t));
            break;
          }
          case P_GPIO_CMD_SET_PIN_PUD:
          {
            ...
            send_reply(pfds[i].fd, P_GPIO_REPLY_SET_PIN_PUD, &status, sizeof(int32_t));
            break;
          }
          case P_GPIO_CMD_READ:
          {
            ...
            status = p_d->ops->read(cmd_head.pin, &value);
            if (!status) {
                status = value;
            }
            send_reply(pfds[i].fd, P_GPIO_REPLY_READ, &status, sizeof(int32_t));
            break;
          }
          case P_GPIO_CMD_WRITE:
          {
            ...
            send_reply(pfds[i].fd, P_GPIO_REPLY_WRITE, &status, sizeof(int32_t));
            break;
          }
          case P_GPIO_CMD_SET_PWM:
          {
            ...
            status = p_d->ops->set_pwm(cmd_head.pin,
                           ...
                           cmd_data.pwm_cfg.range,
                           cmd_data.pwm_cfg.duty);
            send_reply(pfds[i].fd, P_GPIO_REPLY_SET_PWM, &status, sizeof(int32_t));
            break;
          }
          default:
            send_reply(pfds[i].fd, P_GPIO_REPLY_UNDEFINED_CMD, NULL, 0);
        }
      }
...
static void init_server(gpio_data *p_d)
{
  int ret;
  ...
  strcpy(server_sockaddr.sun_path, DEFAULT_GPIO_SOCK_PATH);
  ...
  chmod(DEFAULT_GPIO_SOCK_PATH, 0777);
}
...
int main(void)
{
  ...
  rpi_gpio_backend_destroy(&p_d.ops);
  close(p_d.server_sock);
  return 0;
}
