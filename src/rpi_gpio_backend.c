...
const unsigned rpi_gpio_pins[] = {2, 3, 4, 14, 15, 17, 18, 27, 22, 23, 24, 10, 9, 25, 11, 8, 7, 5, 6, 12, 13, 19, 16, 26, 20, 21};
...
static int rpi_set_mode(unsigned pin, p_gpio_pin_mode mode) {
    unsigned pin_it = 0;
    for (; pin_it < sizeof(rpi_gpio_pins)/sizeof(unsigned); pin_it++) {
        if (rpi_gpio_pins[pin_it] == pin) {
            break;
        }
    }
    if (pin_it == sizeof(rpi_gpio_pins)/sizeof(unsigned)) {
        return -ENODEV;
    }
...
static int rpi_read(unsigned pin)
{
